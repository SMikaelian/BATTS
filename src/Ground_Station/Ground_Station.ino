...
//   over the USB port (Serial3 for hardware pins on Leonardo etc.)
...
void getRPY() {                                    // Function storing roll, pitch & yaw in arrRPY array
  ...
  while (Serial3.available() > 0) {                // While data is available in the serial buffer
    ...
    while (byteRead > 60 || byteRead < 60) {       // If the byte value is greater than or less that 60
      byteRead = Serial3.read();                   // keep reading the byte stream until 60 is found, ie. "<"
    }
    if (byteRead == 60) {                          // If "<" found
      for (int i = 0 ; i <= 2 ; i++) {             // I expected 3 values, so I looped 3 times
        arrRPY[i] = Serial3.parseFloat();          // Write the parsed floating point value found in the serial stream to arrRPY
      }
    }
